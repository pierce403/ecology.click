...
export class GameScene extends Phaser.Scene {
  ...
  private handleTileInteraction(gx: number, gy: number) {
    ...
    if (gx === this.state.player.pos.x && gy === this.state.player.pos.y) {
      if (this.collectResourceAt(gx, gy)) {
        return;
      }
    }
  ...
  private collectResource() {
    this.collectResourceAt(this.state.player.pos.x, this.state.player.pos.y);
  }

  private collectResourceAt(tileX: number, tileY: number): boolean {
    const resource = this.state.resources.find(r =>
      r.pos.x === tileX && r.pos.y === tileY && r.amount > 0
    );
  ...
  private setMovementTarget(tileX: number, tileY: number) {
    ...
    if (current.x === tileX && current.y === tileY) {
      this.movementPath = [];
      this.movementAccum = 0;
      this.collectResourceAt(tileX, tileY);
      return;
    }
